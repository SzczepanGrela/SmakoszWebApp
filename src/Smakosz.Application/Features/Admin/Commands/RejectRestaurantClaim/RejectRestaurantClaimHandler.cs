...
public class RejectRestaurantClaimHandler : IRequestHandler<RejectRestaurantClaimCommand, ErrorOr<Success>>
{
    private readonly ISmakoszDbContext _db;
    private readonly ICurrentUserService _currentUser;

    public RejectRestaurantClaimHandler(ISmakoszDbContext db, ICurrentUserService currentUser)
    {
        _db = db;
        _currentUser = currentUser;
    }

    public async Task<ErrorOr<Success>> Handle(RejectRestaurantClaimCommand request, CancellationToken cancellationToken)
    {
        ...
        if (ticket.RequesterId.HasValue)
        {
            ...
            _db.Notifications.Add(new Notification
            {
                ...
                Title = "Zgłoszenie przejęcia odrzucone",
                Message = $"Twoje zgloszenie przejecia restauracji zostalo odrzucone. Powód: {request.Reason}",
                SendEmail = false,
