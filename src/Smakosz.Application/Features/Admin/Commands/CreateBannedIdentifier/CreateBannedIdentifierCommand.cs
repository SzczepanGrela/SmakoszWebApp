...
public class CreateBannedIdentifierValidator : AbstractValidator<CreateBannedIdentifierCommand>
{
    ...
    {
        RuleFor(x => x.Value)
            .NotEmpty().WithMessage("Wartosc jest wymagana")
            .MaximumLength(255).WithMessage("Wartosc moze miec maksymalnie 255 znakow");

        RuleFor(x => x.Reason)
            .MaximumLength(500).WithMessage("Powod moze miec maksymalnie 500 znakow")
