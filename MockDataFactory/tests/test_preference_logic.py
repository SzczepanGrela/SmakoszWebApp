...
# TEST GROUP 1: Basic Affinity Calculation
...
# TEST GROUP 3: Integration & Fallback
...
# TEST GROUP 4: Weight System Tests (v2.0)
...
def test_reproducible_noise():
    ...
    try:
        ...
        # Should be identical
        for i, (n1, n2) in enumerate(zip(noise1, noise2)):
            assert n1 == n2, f"Noise not reproducible at index {i}: {n1} != {n2}"
        
        logger.info(f"   [OK] Noise is reproducible across runs")
        ...
        logger.info(f"   [OK] Different users get different noise")
