...
# Multi-Size Image Generation Tests
...
class TestResizeAndCrop:
    """Tests for the _resize_and_crop helper method."""
    ...
    def test_resize_and_crop_landscape_to_target(self):
        ...
        # Create a test landscape image (1600x1200)
        img = Image.new("RGB", (1600, 1200), color="red")
        
        # Resize to target 800x600
        result = downloader._resize_and_crop(img, (800, 600))
    ...
    def test_resize_and_crop_portrait_to_target(self):
        ...
        # Create a test portrait image (1200x1600)
    ...
    def test_resize_and_crop_square_to_avatar(self):
        ...
        # Create a test square image (500x500)
        img = Image.new("RGB", (500, 500), color="green")
        
        # Resize to avatar size (300x300)
        result = downloader._resize_and_crop(img, fp.SIZE_AVATAR)
...
class TestMultiSizeProcessing:
    ...
    def test_multi_size_generates_correct_files(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.object(fp.PixabayDownloader, '_get_api_key', return_value='test_key'):
                downloader = fp.PixabayDownloader()
                # Mock config output dir to avoid path issues
                # (The downloader uses GLOBAL OUTPUT_DIR but we pass save_path_full explicitly)
                
                with patch.object(downloader.session, 'get', return_value=mock_response):
                    # We need to mock OUTPUT_DIR in the module because process_image_multi_size uses it 
                    # for relative path calculation
                    with patch('tools.fetch_photos.OUTPUT_DIR', output_dir):
