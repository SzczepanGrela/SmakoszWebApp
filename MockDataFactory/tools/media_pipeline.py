...
from tools.image_providers import ProviderManager, ImageResult, RateLimitError
...
# Predefined extra_sizes bundles for convenience
EXTRA_SIZES_STANDARD = {SUFFIX_THUMB: SIZE_THUMB}            # dishes, restaurants
EXTRA_SIZES_AVATAR = {SUFFIX_THUMB: SIZE_THUMB, SUFFIX_TINY: SIZE_TINY}  # avatars
...
class MediaPipeline:
    ...
    def run_ingredients(self, dry_run: bool = False):
        ...
        for ing_name, search_term, safe_name, ing_dir in tqdm(to_process, desc="Ingredients"):
            ...
            for i, result in enumerate(results[:needed]):
                ...
                self._download_and_process(result.url, save_path, SIZE_INGREDIENT)
    ...
    def run_dishes(self, dry_run: bool = False):
        ...
        max_photos = int(PHOTO_CONFIG["max_photos_per_variant"])
        ...
        for category, data in self.dish_variants.items():
            ...
            for variant_name, variant_data in variants.items():
                ...
                for i, result in enumerate(results):
                    ...
                    self._download_and_process(result.url, save_path, SIZE_FULL, EXTRA_SIZES_STANDARD)
    ...
    def run_avatars(self, dry_run: bool = False):
        ...
        for query in queries:
            ...
            for result in results:
                ...
                self._download_and_process(result.url, save_path, SIZE_AVATAR, EXTRA_SIZES_AVATAR)
                idx += 1
    ...
    def run_hero(self, dry_run: bool = False):
        ...
        for query in HERO_QUERIES:
            ...
            for result in results:
                ...
                if self._download_and_process(result.url, save_path, SIZE_HERO):
    ...
    def _download_and_process(
        ...
        extra_sizes: dict[str, tuple[int, int]] | None = None
    ) -> bool:
        """Download image, resize/crop to target_size, and save.
        
        Also generates derived sizes (thumb, tiny) if extra_sizes is provided.
        
        Args:
            url: Image URL to download
            save_path: Path for the full-size image
            target_size: (width, height) for the primary image
            extra_sizes: Optional dict of suffix -> (width, height) for derived sizes,
                         e.g. {"_thumb": (200, 150), "_tiny": (50, 50)}
        """
        import requests
        from io import BytesIO
        
        try:
            # Add size limit for Unsplash images to avoid decompression bomb
            download_url = url
            if "unsplash.com" in url:
                # Request max 1600px wide - server-side resize
                separator = "&" if "?" in url else "?"
                download_url = f"{url}{separator}w=1600&q=85"
            ...
            img_full = self._resize_and_crop(img.copy(), target_size)
            img_full.save(save_path, "WEBP", quality=85)
            
            # Generate derived sizes (thumb, tiny, etc.)
            if extra_sizes:
                for suffix, size in extra_sizes.items():
                    derived_path = save_path.parent / f"{save_path.stem}{suffix}{save_path.suffix}"
                    img_derived = self._resize_and_crop(img.copy(), size)
                    img_derived.save(derived_path, "WEBP", quality=85)
    ...
    def _resize_and_crop(self, img: Image.Image, target: tuple[int, int]) -> Image.Image:
